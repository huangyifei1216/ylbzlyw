...
import { cancelWish, canScheduleWish, completeWish, createWish, scheduleWish } from "../wish-engine.mjs";

const agreement = { id: "agreement-a", childId: "child-a", recordMode: "daily", startDate: "2026-08-25", endDate: "2026-08-31" };
...
assert.throws(() => cancelWish(completed.wishes, scheduled.transactions, "wish-a"), error("completed-wish"));
...
const cancelled = cancelWish(scheduled2.wishes, scheduled2.transactions, "wish-c", { cancelledAt: at(26, 2) });
assert.equal(cancelled.wish.status, "cancelled");
assert.equal(walletFor(cancelled.transactions, "child-a").available, 3);
assert.equal(walletFor(cancelled.transactions, "child-a").totalEarned, 3);
assert.throws(() => cancelWish(cancelled.wishes, cancelled.transactions, "wish-c"), error("wish-not-scheduled"));
