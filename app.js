...
import { checkAgreementSafety, getSafetyBoundary } from "./safety.mjs";
import { APP_CONFIG } from "./config.mjs";

const app = document.querySelector("#app");
const toast = document.querySelector("#toast");
const storage = createStorageAdapter();
const demoAccess = APP_CONFIG.environment === "development" ? globalThis.__YLB_DEMO_ACCESS__ || null : null;
...
function renderHome(child) {
  ...
  if (agreement?.status === "active") {
    ...
    if (updated.status !== agreement.status) { replaceAgreement(updated); void save().then((ok) => { if (!ok) render(); }); agreement = updated; }
  }
...
function renderReview(child) {
  const agreement = [...state.agreements].reverse().find((item) => item.childId === child.id && item.status === "review-due"); if (!agreement) return simplePage("现在没有需要回顾的约定", "新的约定结束后，会在这里一起看看。", "返回今天", "home");
...
function renderModal() {
  ...
  if (modal.type === "choose-wish") return modalShell(`<h2>换一个全家都期待的心愿</h2><p>这是全家一起安排的体验，不是孩子服从后才有的奖品。</p><form id="wish-form"><input type="hidden" name="childId" value="${h(modal.childId)}"><div class="wish-picker">${FAMILY_WISHES.map((item) => `<label><input type="radio" name="wishId" value="${item.id}" ${item.id === "movie" ? "checked" : ""}><span><i>${h(item.icon)}</i><b>${h(item.title)}</b><small>${item.cost}颗象果</small></span></label>`).join("")}</div><label class="field"><span>自定义心愿（选择“自己定一个”时填写）</span><input name="customWishTitle" minlength="2" maxlength="30" placeholder="例如：周末一起去江边散步"></label>${errorHtml()}<button class="brand-button full-button">确认新的家庭心愿</button></form>`);
  ...
  if (modal.type === "handbook-blocked") return modalShell(`<h2>新窗口没有打开</h2><p>浏览器拦截了新窗口。当前问题已经尝试复制，你可以在当前页面继续打开分龄手册。</p><a class="brand-button full-button handbook-fallback" href="${h(modal.url)}">在当前页面打开分龄手册</a><button class="secondary-button full-button" data-action="close-modal">先留在一两步</button>`);
...
async function onClick(event) {
  ...
  if (action === "start-onboarding") { state.bridgeSeen = true; if (!await save()) return render(); return navigate("onboarding"); }
  if (action === "add-child") return navigate("onboarding");
  if (action === "select-child") { state.currentChildId = target.dataset.id; if (!await save()) return render(); return navigate("home"); }
...
async function onSubmit(event) {
  ...
  if (event.target.id === "wish-form") return createReplacementWish(String(data.get("childId") || ""), String(data.get("wishId") || ""), String(data.get("customWishTitle") || ""));
}

async function onChange(event) {
  if (event.target.id === "child-switcher") { state.currentChildId = event.target.value; if (!await save()) return render(); return navigate("home"); }
  if (event.target.closest("#cycle-form") && ["duration", "wishId", "customWishTitle"].includes(event.target.name)) {
...
async function activate(code) { if (!demoAccess) { formError = "公开预览不接受本地测试码，请使用项目方提供的正式开通方式。"; return render(); } const result = demoAccess.validateDemoCode(code); if (!result.ok) { formError = result.message; return render(); } state = { ...emptyState(), entitlement: result.entitlement }; formError = ""; if (!await save()) return render(); navigate("bridge"); }
async function addChild(nickname, birthDate) { const clean = nickname.trim(); const info = getAgeInfo(birthDate); if (!clean || !info.valid) { formError = info.reason || "请填写完整信息。"; return render(); } if (state.children.length >= childLimit(state.entitlement)) { formError = "当前版本的孩子档案数量已满。"; return render(); } if (!info.graduated && !hasStageAccess(state.entitlement, info.stage.id)) { formError = `孩子目前属于「${info.stage.label}」，与已购阶段不一致。`; return render(); } const child = { id: uid("child"), nickname: clean, birthDate, createdAt: new Date().toISOString() }; state.children.push(child); state.currentChildId = child.id; state.bridgeSeen = true; if (!await save()) return render(); startDraft(child); navigate("create"); }
...
function saveActions(problem, childAction, parentAction) { if ([problem, childAction, parentAction].some((value) => value.trim().length < 4)) { formError = "请把问题和双方行动写得更具体一些。"; return render(); } const safety = checkAgreementSafety({ problem, childAction, parentAction }); if (safety.blocked) { blockedProblem = { problem, childAction, parentAction }[safety.field] || problem; return navigate("safety"); } draft = { ...draft, step: 3, problem: problem.trim(), childAction: childAction.trim(), parentAction: parentAction.trim() }; formError = ""; render(); }
...
async function confirmAgreementDraft() {
  try {
    ...
    if (!await save(nextState)) return render(); draft = null; showToast("家庭约定已经开始"); navigate("home");
  } catch (error) { showToast(error.message); }
}

async function recordAction(agreementId, role) { try { const agreement = state.agreements.find((item) => item.id === agreementId); const result = recordStep({ records: state.records, transactions: state.fruitTransactions, petPeaks: state.petPeaks }, { agreement, role, localDate: dateKeyInShanghai(), recordedAt: new Date() }); state.records = result.records; state.fruitTransactions = result.transactions; state.petPeaks = result.petPeaks; if (!await save()) return render(); showToast(result.duplicate ? "这一轮已经记过了" : result.addedTransactions.length === 2 ? "这一小步和同行象果都记下了" : "这一小步已经记下"); render(); } catch (error) { showToast(error.message); } }
async function undoAction(recordId) { try { const result = reverseRecord({ records: state.records, transactions: state.fruitTransactions, petPeaks: state.petPeaks }, { recordId, reversedAt: new Date() }); state.records = result.records; state.fruitTransactions = result.transactions; if (!await save()) return render(); showToast("误操作已撤回，象果同步调整"); render(); } catch (error) { showToast(error.message); } }
async function scheduleWishAction(wishId, scheduledDate) { try { const result = scheduleWish(state.wishes, state.fruitTransactions, wishId, { scheduledDate }); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!await save()) return render(); modal = null; showToast("家庭心愿已经安排好"); render(); } catch (error) { formError = error.message; render(); } }
async function completeWishAction(wishId) { try { const result = completeWish(state.wishes, wishId); state.wishes = result.wishes; if (!await save()) return render(); showToast("这个心愿已收进家庭回忆"); render(); } catch (error) { showToast(error.message); } }
async function unscheduleWishAction(wishId) { if (!confirm("取消这次安排并全额返还象果吗？心愿会继续保留。")) return; try { const result = unscheduleWish(state.wishes, state.fruitTransactions, wishId); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!await save()) return render(); showToast("这次安排已取消，心愿和象果都保留"); render(); } catch (error) { showToast(error.message); } }
async function abandonWishAction(wishId) { if (!confirm("确定放下这个家庭心愿，并马上选择一个新心愿吗？")) return; try { const result = abandonWish(state.wishes, state.fruitTransactions, wishId); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!await save()) return render(); modal = { type: "choose-wish", childId: result.wish.childId }; formError = ""; showToast("旧心愿已放下，现在选一个新的"); render(); } catch (error) { showToast(error.message); } }
async function createReplacementWish(childId, wishId, customTitle) { try { const option = FAMILY_WISHES.find((item) => item.id === wishId); if (!option) throw new Error("请选择一个家庭心愿。"); const title = wishId === "custom" ? customTitle.trim() : option.title; const made = createWish(state.wishes, { childId, title, icon: option.icon, cost: option.cost }); state.wishes = made.wishes; if (!await save()) return render(); modal = null; showToast("新的家庭心愿已经开始积累"); render(); } catch (error) { formError = error.message; render(); } }
async function finishReview(id, outcome) { try { const agreement = state.agreements.find((item) => item.id === id); const reviewed = reviewAgreement(agreement, outcome); replaceAgreement(reviewed); if (!await save()) return render(); if (outcome === "pause") { showToast("这一轮先放一放"); return navigate("home"); } const child = currentChild(); const info = getAgeInfo(child.birthDate); startDraft(child); if ((outcome === "continue" || outcome === "adjust") && agreement.stageId !== info.stage?.id) { showToast("孩子已经进入新的成长阶段，我们从这个阶段重新选一个当前问题。"); return navigate("create"); } if (outcome === "continue" || outcome === "adjust") { draft = { ...draft, step: outcome === "continue" ? 3 : 2, problem: agreement.problem, childAction: agreement.childAction, parentAction: agreement.parentAction, templateId: agreement.templateId, templateVersion: agreement.templateVersion, duration: agreement.duration, wishId: currentWish(child.id)?.id || "movie" }; } showToast("这一轮已收进过去"); navigate("create"); } catch (error) { showToast(error.message); } }
...
async function confirmImport() { try { state = applyPreparedBackup(state, modal.prepared, { confirmed: true }); if (!await save()) return render(); modal = null; showToast("家庭备份已经导入"); navigate("home"); } catch (error) { showToast(error.message); } }
async function deleteChild(id) { state.children = state.children.filter((item) => item.id !== id); const agreementIds = new Set(state.agreements.filter((item) => item.childId === id).map((item) => item.id)); state.agreements = state.agreements.filter((item) => item.childId !== id); state.records = Object.fromEntries(Object.entries(state.records).filter(([, item]) => item.childId !== id && !agreementIds.has(item.agreementId))); state.fruitTransactions = state.fruitTransactions.filter((item) => item.childId !== id); state.wishes = state.wishes.filter((item) => item.childId !== id); delete state.petPeaks[id]; state.currentChildId = state.children[0]?.id || ""; if (!await save()) return render(); modal = null; navigate(state.children.length ? "profile" : "onboarding"); }
async function resetApp() { if (state.entitlement && !confirm("确定清空当前浏览器里的一两步数据吗？建议先导出家庭备份。")) return; const result = await storage.clear(); state = result.state; storageWarning = !result.ok; if (!result.ok) { const unknown = result.error === "unknown-commit-state"; formError = unknown ? "浏览器无法确认清空是否完成，请立即刷新页面核对；不要继续录入新数据。" : "未能清空本机数据，当前家庭资料仍然保留。请导出备份后更换普通浏览模式再试。"; showToast(unknown ? "清空状态无法确认，请刷新核对" : "清空失败，家庭资料没有变化"); return render(); } draft = null; modal = null; formError = ""; location.hash = "#/activate"; render(); }

async function seedDemo() { if (!demoAccess) return; const entitlement = { scope: "all", stageId: "all", label: "0—18岁全龄陪伴卡", status: "active" }; const child = { id: "demo-feifei", nickname: "菲菲", birthDate: "2020-05-18", createdAt: new Date().toISOString() }; state = { ...emptyState(), entitlement, children: [child], currentChildId: child.id, bridgeSeen: true, demo: true }; startDraft(child); draft = { ...draft, step: 4, templateId: "s3-morning", problem: "早上出门总磨蹭", childAction: "起床后先完成自己选的第一项准备", parentAction: "我只提醒一次，并给两个可接受的选择", duration: 7, wishId: "movie" }; if (await save()) await confirmAgreementDraft(); }
async function activateFromMagicLink() { if (!demoAccess) return; const code = new URLSearchParams(location.search).get("code"); if (!code || state.entitlement) return; const result = demoAccess.validateDemoCode(code); if (result.ok) { state.entitlement = result.entitlement; if (await save()) history.replaceState({}, "", `${location.pathname}#/bridge`); } }
async function openHandbook(query = "") {
  const text = query || blockedProblem || "请结合孩子年龄，告诉我现场第一步、具体话术和需要观察的情况。";
  const opened = APP_CONFIG.handbookUrl ? window.open(APP_CONFIG.handbookUrl, "_blank", "noopener") : null;
...
async function save(candidate = state) { const result = await storage.write(candidate); state = result.state; const conflict = result.error === "storage-conflict"; storageWarning = !result.ok && !conflict; if (!result.ok) { const unknown = result.error === "unknown-commit-state"; formError = conflict ? "另一页面刚刚更新了家庭资料，本页这次操作没有覆盖它。请确认最新内容后再操作。" : unknown ? "浏览器无法确认这次保存是否完成，请立即刷新页面核对；不要继续录入新数据。" : "这次操作没有保存，页面已恢复到上一次成功保存的状态。请导出备份或更换普通浏览模式。"; showToast(conflict ? "已保留另一页面的最新更新" : unknown ? "保存状态无法确认，请刷新核对" : "未能保存，这次操作已撤回"); } return result.ok; }
