...
const demoAccess = APP_CONFIG.environment === "development" ? await import("./demo-access.mjs") : null;
...
function renderHome(child) {
  ...
  if (agreement?.status === "active") {
    ...
    if (updated.status !== agreement.status) { replaceAgreement(updated); if (!save()) return persistenceFailureHtml(); agreement = updated; }
  }
...
async function onClick(event) {
  ...
  if (action === "start-onboarding") { state.bridgeSeen = true; if (!save()) return render(); return navigate("onboarding"); }
  if (action === "add-child") return navigate("onboarding");
  if (action === "select-child") { state.currentChildId = target.dataset.id; if (!save()) return render(); return navigate("home"); }
...
async function onChange(event) {
  if (event.target.id === "child-switcher") { state.currentChildId = event.target.value; if (!save()) return render(); return navigate("home"); }
...
function activate(code) { if (!demoAccess) { formError = "公开预览不接受本地测试码，请使用项目方提供的正式开通方式。"; return render(); } const result = demoAccess.validateDemoCode(code); if (!result.ok) { formError = result.message; return render(); } state = { ...emptyState(), entitlement: result.entitlement }; formError = ""; if (!save()) return render(); navigate("bridge"); }
function addChild(nickname, birthDate) { const clean = nickname.trim(); const info = getAgeInfo(birthDate); if (!clean || !info.valid) { formError = info.reason || "请填写完整信息。"; return render(); } if (state.children.length >= childLimit(state.entitlement)) { formError = "当前版本的孩子档案数量已满。"; return render(); } if (!info.graduated && !hasStageAccess(state.entitlement, info.stage.id)) { formError = `孩子目前属于「${info.stage.label}」，与已购阶段不一致。`; return render(); } const child = { id: uid("child"), nickname: clean, birthDate, createdAt: new Date().toISOString() }; state.children.push(child); state.currentChildId = child.id; state.bridgeSeen = true; if (!save()) return render(); startDraft(child); navigate("create"); }
...
function recordAction(agreementId, role) { try { const agreement = state.agreements.find((item) => item.id === agreementId); const result = recordStep({ records: state.records, transactions: state.fruitTransactions, petPeaks: state.petPeaks }, { agreement, role, localDate: dateKeyInShanghai(), recordedAt: new Date() }); state.records = result.records; state.fruitTransactions = result.transactions; state.petPeaks = result.petPeaks; if (!save()) return render(); showToast(result.duplicate ? "这一轮已经记过了" : result.addedTransactions.length === 2 ? "这一小步和同行象果都记下了" : "这一小步已经记下"); render(); } catch (error) { showToast(error.message); } }
function undoAction(recordId) { try { const result = reverseRecord({ records: state.records, transactions: state.fruitTransactions, petPeaks: state.petPeaks }, { recordId, reversedAt: new Date() }); state.records = result.records; state.fruitTransactions = result.transactions; if (!save()) return render(); showToast("误操作已撤回，象果同步调整"); render(); } catch (error) { showToast(error.message); } }
function scheduleWishAction(wishId, scheduledDate) { try { const result = scheduleWish(state.wishes, state.fruitTransactions, wishId, { scheduledDate }); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!save()) return render(); modal = null; showToast("家庭心愿已经安排好"); render(); } catch (error) { formError = error.message; render(); } }
function completeWishAction(wishId) { try { const result = completeWish(state.wishes, wishId); state.wishes = result.wishes; if (!save()) return render(); showToast("这个心愿已收进家庭回忆"); render(); } catch (error) { showToast(error.message); } }
function unscheduleWishAction(wishId) { if (!confirm("取消这次安排并全额返还象果吗？心愿会继续保留。")) return; try { const result = unscheduleWish(state.wishes, state.fruitTransactions, wishId); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!save()) return render(); showToast("这次安排已取消，心愿和象果都保留"); render(); } catch (error) { showToast(error.message); } }
function abandonWishAction(wishId) { if (!confirm("确定放下这个家庭心愿吗？之后可以重新选择一个新心愿。")) return; try { const result = abandonWish(state.wishes, state.fruitTransactions, wishId); state.wishes = result.wishes; state.fruitTransactions = result.transactions; if (!save()) return render(); showToast("这个心愿已放下，可以重新选择"); render(); } catch (error) { showToast(error.message); } }
function finishReview(id, outcome) { try { const agreement = state.agreements.find((item) => item.id === id); const reviewed = reviewAgreement(agreement, outcome); replaceAgreement(reviewed); if (!save()) return render(); if (outcome === "pause") { showToast("这一轮先放一放"); return navigate("home"); } const child = currentChild(); const info = getAgeInfo(child.birthDate); startDraft(child); if ((outcome === "continue" || outcome === "adjust") && agreement.stageId !== info.stage?.id) { showToast("孩子已经进入新的成长阶段，我们从这个阶段重新选一个当前问题。"); return navigate("create"); } if (outcome === "continue" || outcome === "adjust") { draft = { ...draft, step: outcome === "continue" ? 3 : 2, problem: agreement.problem, childAction: agreement.childAction, parentAction: agreement.parentAction, templateId: agreement.templateId, templateVersion: agreement.templateVersion, duration: agreement.duration, wishId: currentWish(child.id)?.id || "movie" }; } showToast("这一轮已收进过去"); navigate("create"); } catch (error) { showToast(error.message); } }
